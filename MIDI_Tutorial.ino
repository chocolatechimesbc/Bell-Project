...
void setup() {
  MIDI.begin(1);
  pinMode(6, OUTPUT);     //striker
  digitalWrite(6, LOW);
  pinMode(7, OUTPUT);     //damper
  pinMode(8, OUTPUT);     //mag switch
  
}

void loop() {
  //Code below is for using the piano as a synth
  ...
  //}
   if (MIDI.read(1));    //reads bytes from piano keyboard 
    {
      digitalWrite(6, HIGH);
      digitalWrite(7, HIGH);
      digitalWrite(8, HIGH);
